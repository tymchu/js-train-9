...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = arr.findIndex((item) => item.name === name);
  return foundIndex;
}
...
function findLastEvenNumber(arr) {
  ...
  const foundLast = arr.findLast((num) => num % 2 === 0);
  if (!foundLast) {
...
function getPositiveArrayValues(arr) {
  ...
  return Array.from(arr.values()).filter((item) => item > 0);
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  slicedArr.forEach((item) => console.log(item));
  return slicedArr.reduceRight((accum, item) => (accum += item));
}
...
function expandArrayByN(arr, n) {
  ...
  const expandedArray = arr.flatMap((item) => {
    let itemArray = [];
    for (let i = 1; i <= n; i++) {
      itemArray.push(item);
    }
...
function findDuplicateElements(arr) {
  ...
  return arr.filter((item, currentIndex) => arr.indexOf(item) !== currentIndex);
}
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((accum, currentItem) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  return arr.findLastIndex((item) => item < 0);
}
